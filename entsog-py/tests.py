...
def get_area():
    ...
    data = json.loads(client.query_operator_point_directions())
...
for c in Country:
    try:
        country = c.code
        print(country)
        data = client.query_aggregated_data(start = start, end = end, country_code = country, period_type=PERIOD_TYPE) #, balancing_zone= country, group_type = 'country', entry_exit = False)
        print(data)
